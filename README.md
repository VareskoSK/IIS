...
- `PostgreSQL` + `pgAdmin` — база данных (для мониторинга data shift)
...
- **`grafana/`**
  - `my_dashboard.json` — экспорт дашборда с 5 графиками
...
## Визуализация мониторинга (prometheus_client)
![Предсказания модели по батчам](image_monitoring/гистограмма по батчам.jpg)

![Количество ошибок при запросах к серверу](image_monitoring/колво ошибок.jpg)

![Частота запросов к серверу](image_monitoring/частота запросов к серверу.jpg)

## 📊 Дашборд мониторинга (Grafana)

![Описание графиков](image_monitoring/дашборд графана.jpg)
